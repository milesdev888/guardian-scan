...
## v2.1 — Composite score + LP lock detection

- **Composite score:** weighted average of check grades (A=100 … F=0). Weights: honeypot 20, LP lock 20, holders 20, mint/freeze 15, transfer tax 10, age 10, copycats 5. Grade **U is excluded** from the denominator. Overall: 85+ A · 70–84 B · 50–69 C · 30–49 D · &lt;30 F.
- **LP lock:** reads per-pool lock % from RugCheck markets (Meteora DAMM v2 permanent lock, Raydium/Orca burn/lock, Streamflow-held LP). UI lock icon: green ≥90% · gold 50–89% · gray/red below or unknown. Grades: ≥90 A · 50–89 B · 1–49 C · 0 D · undetectable **U** (not F).
- **Holder concentration:** classifies top holders (LP/pool → Liquidity, Streamflow/vesting → Locked, burns excluded). Shows raw top-10 **and** adjusted top-10; grade keys off the adjusted figure.

Acceptance mint: `$C7` `979sitxCjWFPdAsrF2ybKNENwFcpiHDwaAasC5Xa5qww` — expect green 🔒 100% LP lock, adjusted holder grade, composite **B or better**.
