      ...
      holders.length === 0
        ...
        : freeFloatHolders.length === 0
          ? check({
              ...
              status: "pass",
              grade: "A",
              summary: `Top 10 wallets hold ${formatPct(concentration.rawTop10)} of supply, all excluded as burns/locks.`,
              detail: `Every top-10 wallet is tagged as a burn or lock address, so ${formatPct(concentration.excludedPct)} is excluded from free-float math.`,
          ...
          : top10 >= 70
            ? check({
                ...
                detail: `Raw top-10 holds ${formatPct(concentration.rawTop10)}; ${formatPct(concentration.excludedPct)} was excluded as burns, locks, or escrows before scoring free-float concentration.`,
                evidence: { ...concentration, holders },
              })
            : check({
                id: "holder_concentration",
                ...
                grade: top10 >= 50 ? "B" : "A",
                summary: `Top free-float wallets hold ${formatPct(top10)} of supply.`,
                detail: `${freeFloatHolders.length} free-float accounts scored; raw top-10 was ${formatPct(concentration.rawTop10)} with ${formatPct(concentration.excludedPct)} excluded as burns, locks, or escrows.`,
                evidence: { ...concentration, holders },
              }),
    );
